...
class BaseNewsSource(ABC):
    ...
    def __init__(self, url: str):
        self.parsed_source = BeautifulSoup(
            requests.get(url=url, headers=self._get_headers()).content, features="lxml"
        )
    ...
    def create_mem_from_photo(
        ...
        black_im = Image.new(
            "RGBA", (width, height), color=NewsColorsAndFonts.GRADIENT_COLOR
        )  # i.e. black
        ...
        total_text_height, wrapped_text, padding = cls._wrap_text_and_get_total_height(
            image=img, text=news.title, title_font=title_font, source_font=info_font
        )

        new_image_height = int(img.size[1] + total_text_height)
        overlay = Image.new(
            "RGBA", (img.size[0], new_image_height), NewsColorsAndFonts.GRADIENT_COLOR
        )
        ...
        d.text(
            ((img.size[0] - width_source) / 2, txt_height),
            cls.SOURCE,
            font=fnt_mir,
    ...
    def _is_duplicated_news(one_news: News) -> bool:
        ...
        threshold = float(os.getenv("DEDUPLICATION_THRESHOLD") or 0.3)
